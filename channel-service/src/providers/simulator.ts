...
async function runProgression(communicationId: string, providerMessageId: string): Promise<void> {
  try {
    await fire(communicationId, providerMessageId, 'SENT', QUEUED_TO_SENT_DELAY);

    if (Math.random() * 100 < FAILURE_RATE) {
      await fire(communicationId, providerMessageId, 'FAILED', SENT_TO_DELIVERED_DELAY);
      return;
    }

    await fire(communicationId, providerMessageId, 'DELIVERED', SENT_TO_DELIVERED_DELAY);

    if (Math.random() < 0.6) {
      await fire(communicationId, providerMessageId, 'READ', DELIVERED_TO_READ_DELAY);

      if (Math.random() < 0.2) {
        await fire(communicationId, providerMessageId, 'CLICKED', READ_TO_CLICKED_DELAY);
      }
    }
  } catch (err) {
...
export class SimulatorProvider implements ChannelProvider {
  async send(request: SendRequest): Promise<string> {
    ...
    runProgression(request.communicationId, providerMessageId).catch((err) => {
