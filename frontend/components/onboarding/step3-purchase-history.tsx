...
export function Step3PurchaseHistory({
  onFileUpload,
  uploadedFile,
  previewData,
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Upload your order data in CSV format
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {!uploadedFile ? (
          ...
          >
            ...
            <p className="text-xs text-gray-500">CSV file with order data</p>
          </div>
        ) : (
                ...
                {previewData && (
                  <div className="mt-2 space-y-1">
                    <div className="text-sm text-green-700">
                      ✓ {previewData.totalOrders.toLocaleString()} orders detected
                    </div>
                    {previewData.dateRange && (
                      <div className="text-xs text-green-600">
                        Date Range: {previewData.dateRange.start} → {previewData.dateRange.end}
                      </div>
                    )}
                  </div>
                )}
